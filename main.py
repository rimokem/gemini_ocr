...
DEFAULT_OCR_PROMPT = "この画像に含まれるすべてのテキストを抽出してください。改行や段落構造を維持し、完全かつ正確に文字起こしをしてください。抽出したテキストのみを出力し、それ以外の説明や応答は含めないでください。以下に追加の指示がある場合は、それを優先して従ってください。"


def parse_arguments() -> argparse.Namespace:
    """コマンドライン引数を解析する"""
    parser = argparse.ArgumentParser(
        description="PDFファイルからテキストを抽出するOCRツール"
    )
    # 必須引数
    ...
    page_group.add_argument(
        "-f", "--first-page", type=int, help="開始ページ番号（1始まり）"
    )
    page_group.add_argument(
        "-l", "--last-page", type=int, help="終了ページ番号（1始まり）"
    )
    ...
    output_group.add_argument(
        "-o",
        "--output",
        help="出力テキストファイルのパス（指定しない場合はPDFファイル名_ocr.txtになります）",
    )
    output_group.add_argument(
        "-k",
        "--keep-images",
        action="store_true",
        help="処理後に一時画像ファイルを保持する",
    )
    ...
    process_group.add_argument(
        ...
        help=f"PDFの拡大率（デフォルト: {DEFAULT_ZOOM}）",
    )
    process_group.add_argument(
        "-p", "--prompt", help="デフォルトのプロンプトに追加するテキスト"
    )
...
def convert_page_to_image(
    ...
    output_file = os.path.join(
        output_dir, f"{pdf_name}_page_{page_num + 1:04d}.{output_format}"
    )
    pix.save(output_file)

    print(
        f"ページ {page_num - page_offset + 1}/{total_pages} を保存しました: {os.path.basename(output_file)}"
    )
    return output_file
...
def convert_pdf_to_images(
    ...
    with fitz.open(pdf_path) as pdf_document:
        ...
        # 各ページに変換関数を適用
        return [
            convert_page(page_num=page_num)
...
def init_genai_client() -> genai.Client:
    ...
    if not api_key:
        raise ValueError(
            "環境変数 'GOOGLE_API_KEY' が設定されていません。.envファイルを確認してください。"
        )
...
def process_image(
    client: genai.Client, image_path: str, index: int, total: int, prompt: str
) -> Tuple[str, str]:
    ...
    try:
        ...
        image_files = sorted(
            [
                ...
                and any(file.lower().endswith(ext) for ext in [".png", ".jpg", ".jpeg"])
            ]
        )
        ...
        with open(output_file_path, "w", encoding="utf-8") as output_file:
            output_file.write(
                "\n".join(
                    [f"------------------\n\n{text}" for filename, text in results]
                )
            )

        print(
            f"OCR処理が完了しました。結果は '{output_file_path}' に保存されています。"
        )
        return True
